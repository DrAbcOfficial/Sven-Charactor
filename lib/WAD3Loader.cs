    ...
    public class WAD3Loader
    {
       ...
       public class QFont
       {
        ...
        public void LoadFile(string inputFile)
        {
            ...
            {
                Close();
                throw new TextureDimensionException("Invalid or unsupported WAD File!");
            }
        ...
        public Texture GetLumpImage(int index, bool transparent = false)
        {
            ...
            if (index > -1 && index < LumpsInfo.Count)
            {
                ...
                {
                    throw new TextureDimensionException("Unsupported type (0x" + LumpsInfo[index].Type.ToString("X") + ") of lump (texture) in WAD!");
                }
            }
            else
            {
        ...
        public static int getSize(in int iInt)
        {
            return Math.Min(iInt, 96);
        }
        ...
        public static void CreateWad(string outputFilename, string[] images, string[] names, bool reserverLastPalColor = false)
        {
            ...
            using (BinaryWriter bw = new BinaryWriter(fs))
            {
                ...
                for (int i = 0; i < images.Length; i++)
                {
                    ...
                    int width = originalImage.Width / 16;
                    int height = originalImage.Height / 16;
                    width = getSize(width * 16);
                    height = getSize(height * 16);

                    originalImage.Rescale(new Size(width, height), FREE_IMAGE_FILTER.FILTER_CATMULLROM);
